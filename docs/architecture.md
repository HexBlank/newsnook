...
| 界面 | 职责 |
|---|---|
| 今日 `FeedScreen` | 分类轨 + 多源混合时间线 + 下拉刷新 + 场景预设切换 |
...
- 主题 `system | light | dark`；**墨水屏** `einkMode`（行为叠加，非第三主题）
- 翻译引擎、语言、呈现方式、云 API 配置、列表标题翻译
- 代理模式与地址、切换分类时自动刷新

主题（`lib/theme.ts`）：解析后写入 `<html data-theme>`。`index.css` 语义色 `--color-ink / --color-paper / …` 统一指向 `--tone-*`；图片查看器与视频播放器局部 `data-theme="dark"`。首屏由 `index.html` 内联脚本先行定色，并同步写入 `data-eink` 防闪。
