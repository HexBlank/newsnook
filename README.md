...
- **本地优先**：偏好、稍后读、阅读记录与正文缓存只存在本机，不上传到任何服务器
...
- **翻译**：云端（Google / Azure / DeepL / DeepLX / OpenAI 兼容）两变体均可用；Android ML Kit 与 Bergamot 离线仅完整版（`local` 包）
...
图片支持放大、缩放；长按可保存与分享。网易视频稿及正文内嵌播放器支持全屏；全屏下半屏可横滑调进度，左下竖滑调亮度，右下竖滑调音量。
...
在阅读器顶栏点「收藏」可加入「稍后读」；加入后会预加载正文，便于无网时阅读。最近阅读过的条目有历史记录。完整打开过的正文会缓存在本机，断网后仍可回看；缓存过多时可在设置中手动清理。
