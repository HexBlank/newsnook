...
- **站内全文阅读**：应用内渲染正文，随时可跳转原站核对；部分源支持跟贴；图片放大 / 保存 / 分享，网易视频稿支持全屏手势调进度、亮度与音量
...
在阅读器顶栏点「收藏」可加入「稍后读」；加入后会预加载正文，便于无网时阅读。最近阅读过的条目有历史记录。完整打开过的正文会缓存在本机，断网后仍可回看；可在「我的 → 离线存储」中按正文/列表分别清理，或一键清除全部可管理缓存。
